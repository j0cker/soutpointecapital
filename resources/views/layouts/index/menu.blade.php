
<!-- start header -->
<header id="main-header" data-height-onload="100" data-height-loaded="true" class="" style="position: absolute; background-color: rgba(255,255,255,0.5); top: 0px;">
    <div style="height: 100px; max-width: 100% !important;" class="container clearfix et_menu_container">
    <div data-aos="zoom-out" class="logo_container" style="position: absolute; width: 100%; height: 100%;">
        <span class="logo_helper"></span>
        <a href="/">
          <img style="padding-top: 0px;" class="logo hvr-pulse" src="images/logo.png" alt="SOUTHPOINTE CAPITAL" id="logo">
        </a>
    </div>
    <div data-aos="zoom-out" id="et-top-navigation" data-height="66" data-fixed-height="60" style="padding-top: 33px; padding-left: 269px;">
        <nav>
        <ul style="" id="top-menu" class="nav">
            <li id="menu-item-229" class="menu-item menu-item-type-post_type menu-item-object-page menu-item-229">
              <a class="aboutUsMenu" style="cursor: pointer !important; background-color: transparent; color: black;">ABOUT US</a>
            </li>
            <li id="menu-item-229" class="menu-item menu-item-type-post_type menu-item-object-page menu-item-229">
              <a class="servicesMenu" style="cursor: pointer !important; background-color: transparent; color: black;">SERVICES</a>
            </li>
            <li id="menu-item-229" class="menu-item menu-item-type-post_type menu-item-object-page menu-item-229">
              <a style="cursor: pointer !important; background-color: transparent; color: black;" href="/people">PEOPLE</a>
            </li>
            <li id="menu-item-255" class="menu-item menu-item-type-post_type menu-item-object-page menu-item-255">
              <a style="cursor: pointer !important; background-color: transparent; color: black;" href="/portfolio">PORTFOLIO</a>
            </li>
            <li id="menu-item-255" class="menu-item menu-item-type-post_type menu-item-object-page menu-item-255">
              <a style="cursor: pointer !important; background-color: transparent; color: black;" href="/news">NEWS</a>
            </li>
            <li id="menu-item-255" class="menu-item menu-item-type-post_type menu-item-object-page menu-item-255">
              <a class="contactsMenu" style="cursor: pointer !important; background-color: transparent; color: black;">CONTACT</a>
            </li>
        </ul>
        </nav>




        <div id="et_mobile_nav_menu">
          <div class="mobile_nav closed">
            <span style="padding-bottom: 24px;" class="mobile_menu_bar mobile_menu_bar_toggle"></span>
          </div>
        </div>
      </div> <!-- #et-top-navigation -->
    </div> <!-- .container -->
</header> <!-- #main-header -->